...
const FarmCard: React.FC<FarmCardProps> = ({ farm, removed, cakePrice, bnbPrice, ethPrice, ethereum, account }) => {
  ...
  const totalValue: BigNumber = useMemo(() => {
    ...
    if (farm.quoteTokenSymbol === QuoteToken.USDC) {
      return new BigNumber(1000000000000).times(farm.lpTotalInQuoteToken)
    }    
    
    if (farm.quoteTokenSymbol === QuoteToken.fUSDT) {
      return new BigNumber(1000000000000).times(farm.lpTotalInQuoteToken)
    }
...
const lpPrice = useMemo(() => {
  ...
  return (
    <FCard>
      {farm.tokenSymbol === 'CLEVER' && <StyledCardAccent />}
      {farm.tokenSymbol === 'TREAT' && <StyledCardAccent />}
      {farm.tokenSymbol === 'UNDEAD' && <StyledCardAccent />}
      <CardHeading
        lpLabel={lpLabel}
      ...
      <CardActionsContainer farm={farm} ethereum={ethereum} account={account} lpPrice={lpPrice} />
